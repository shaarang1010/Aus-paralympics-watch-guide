export const getAllDisciplines = () => {
  return [
    {
      name: 'BLIND FOOTBALL',
      id: 'blind-football',
    },
    {
      name: 'BOCCIA',
      id: 'boccia',
    },
    {
      name: 'GOALBALL',
      id: 'goalball',
    },
    {
      name: 'PARA ARCHERY',
      id: 'para-archery',
    },
    {
      name: 'PARA ATHLETICS',
      id: 'para-athletics',
    },
    {
      name: 'PARA BADMINTON',
      id: 'para-badminton',
    },
    {
      name: 'PARA CANAOE',
      id: 'para-canaoe',
    },
    {
      name: 'PARA CYCLING ROAD',
      id: 'para-cycling-road',
    },
    {
      name: 'PARA CYCLING TRACK',
      id: 'para-cycling-track',
    },
    {
      name: 'PARA EQUESTRIAN',
      id: 'para-equestrian',
    },
    {
      name: 'PARA JUDO',
      id: 'para-judo',
    },
    {
      name: 'PARA POWERLIFTING',
      id: 'para-powerlifting',
    },
    {
      name: 'PARA ROWING',
      id: 'para-rowing',
    },
    {
      name: 'PARA TENNIS TABLE',
      id: 'para-tennis-table',
    },
    {
      name: 'Para TAEKWONDO',
      id: 'para-taekwondo',
    },
    {
      name: 'PARA SWIMMING',
      id: 'para-swimming',
    },
    {
      name: 'PARA TRIATHLON',
      id: 'para-triathlon',
    },
    {
      name: 'SITTING VOLLEYBALL',
      id: 'sitting-volleyball',
    },
    {
      name: 'SHOOTING PARA SPORT',
      id: 'shooting-para-sport',
    },
    {
      name: 'WHEELCHAIR BASKETBALL',
      id: 'wheelchair-basketball',
    },
    {
      name: 'WHEELCHAIR FENCING',
      id: 'wheelchair-fencing',
    },
    {
      name: 'WHEELCHAIR RUGBY',
      id: 'wheelchair-rugby',
    },
    {
      name: 'WHEELCHAIR TENNIS',
      id: 'wheelchair-tennis',
    },
  ];
};
